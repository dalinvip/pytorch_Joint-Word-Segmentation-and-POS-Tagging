...
def init_lstmCell(lstmcell, dim):
    """
    :param lstmcell:
    :param dim:
    :return:
    """
    nn.init.xavier_uniform_(lstmcell.weight_ih)
    nn.init.xavier_uniform_(lstmcell.weight_hh)
    scope = np.sqrt(6 / (dim + 1))
    lstmcell.bias_hh.data.uniform_(-scope, scope)
    lstmcell.bias_ih.data.uniform_(-scope, scope)
...
def init_embedding(input_embedding, dim, paddingKey=None, seed=666):
    """初始化embedding层权重
    """
    # torch.manual_seed(seed)
